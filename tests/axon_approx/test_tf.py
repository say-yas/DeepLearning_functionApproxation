...
from src_nna.axon_approx.src.axon_model_tensorflow import train_random_model_tf
...
for k in tqdm(range(1,10)):
    err_k = train_random_model_tf(xs, lambda x: x**2, K=k, num_epochs=205, num_iters=100, learning_rate=1e-2)
...
plt.plot(errs_rnd, label='Tensorflow')
plt.show()