...
"""
Here, we use pinn algorithm to solve one-dimensional singularly perturbed reaction-diffusion equation 
 -epsilon^2 u''(x) + u(x) =1 
u(0) =u(1) =0
"""
...
def exact_solution(x, eps = 0.1):
    """
    exact solution of the 1D parabolic equation  -eps^2 u''(x) + u(x) =1
    u(x) = A exp(x/eps) + B exp(-x/eps) + 1 
    with 
    A = [1- exp(1/eps)] /[exp(2/eps) -1]
    and 
    B = [exp(1/eps) - exp(2/eps)]/[ exp(2/eps)-1]
    params:
    x: the input space
    eps: float, the parameter of the problem
    """
    A = (1.- exp(1./eps) ) / ( exp(2./eps) -1.)
    B= ( exp(1./eps) - exp(2./eps) ) /( exp(2./ eps) -1)

    return  A * exp(x/eps) + B * exp(-x/eps)+1

def loss_parabolic_1d_eq(model, x0_b, x1_b, x_phys, param):
    """
    loss function for the 1D parabolic equation  -eps^2 u''(x) + u(x) =1
    ...
    lambda1, lambda2, lambda3 = 1.5, 1.0, 1.0
    ...
    # mse loss based on exact solution
    u = model(x_phys)
    loss2 = torch.mean( ( u-torch.tensor( exact_solution(x_phys.detach().numpy(), eps = param[0]) ) )**2)
    ...
    loss = lambda3 * loss3 + lambda1*loss1 + lambda2*loss2
        
    return loss

def train_parabolic_1d_eq(model, lear_rate, x0_b, x1_b, x_phys, x_test, param, num_iter,
                      phys_errors, val_errors, early_stopper=None, plotting=False):  

    """
    Train the PINN model to solve the 1D parabolic equation  -eps^2 u''(x) + u(x) =1
    params:
    model: torch.nn.Module, the neural network model
    ...
    for i in tqdm(range(num_iter+1)):
        ...
        loss = loss_parabolic_1d_eq(model, x0_b, x1_b, x_phys, param)
        loss.backward()
        optimizer.step()
...
def plot_rror(phys_errrs, val_errrs):
    ...
    params:
    phys_errrs: list, the physical errors
...
def run_training(epsilon =0.1,   num_hidden=32, num_layers=3,
    ...
    """
    Run the training of the PINN model to solve the 1D parabolic equation  -eps^2 u''(x) + u(x) =1
    params:
    ...
    loss = loss_parabolic_1d_eq(pinn, x0_boundary, x1_boundary, x_physics, [epsilon])
    train_parabolic_1d_eq(pinn, lear_rate,x0_boundary, x1_boundary, x_physics, x_test, [epsilon],
